...
세부 작업은 이 문서가 아니라 **GitHub Issues + Projects(칸반 보드)**로 관리합니다.
(이 문서는 큰 흐름만, 체크는 칸반에서.)

> v1(첫 런칭) 목표 범위 = **Public 전체**. 족보·스터디 등 Private는 다음 기수.
> 근거는 [DECISIONS.md](./DECISIONS.md) 11번.

---

## Phase 0 — 협업 인프라 마무리
- 초대받은 멤버 초대 수락
- admin/Org Owner 권한 정리 (항상 2명 이상 유지 — 인수인계 안전장치 R1)
- 공개 여부 결정: **public 전환(오픈소스) vs GitHub Education Team(무료 private+브랜치보호) vs 컨벤션 유지**
  - 현재: 무료 + private → 브랜치 보호 규칙 적용 불가, `CONTRIBUTING.md` 컨벤션으로 운영 중
- `HANDOVER.md` 작성 (계정·도메인·결제일·갱신책임·비번 보관 위치 — 안전장치 R2)

## Phase 1 — MVP 골격 구축
- Next.js + TypeScript 초기화 (`output: 'standalone'`)
- 라우팅 + 페이지별 빈 컴포넌트(레이아웃) 구성 (`app/`)
...
## Phase 2 — 이슈 분배 & 작업 시작
- 페이지/기능 단위로 GitHub 이슈 생성 (이슈 템플릿 활용) → Projects 칸반 등록
- 담당자 배정 (Public팀 / Private팀 / 디자인)
...
### Private (다음 기수)
- 멤버 인증 (GitHub Org 멤버십 기반 — ⚠️ 학번·이름 단독 대조는 사칭 위험으로 폐기, DECISIONS 5번)
